...
Неявные касты плохо! Например, есть два класса: метры и секунды. Оба класса могут быть приведены к типу ```int```. И в какой-то момент можно было бы сложить метры и секунды, а это нарушение логики. 
...
При выборе перегрузки работает 3 правила:
